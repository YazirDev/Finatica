  ...
  `);
    ...
    if ((catCount[0]?.values[0][0] || 0) === 0) {
        ...
        categoriasPredefinidas.forEach(([nombre, icono, color, tipo]) => {
            db.run(`
        INSERT INTO categorias (nombre, icono, color, tipo, es_predefinida) 
        VALUES ('${nombre}', '${icono}', '${color}', '${tipo}', 1);
      `);
        });
    }
 ...
 * Guarda automáticamente en disco después de cada operación.
...
function setupIpcHandlers() {
    ...
    ipcMain.handle('recurrentes:toggleActivo', (_, { id, activo }) => {
        run('UPDATE recurrentes SET activo = ? WHERE id = ?', [activo, id]);
        return { ok: true };
    });
